...
@print_log
def inventory_collector_remove_plugin_info_secret_service_account_id_provider(mongo_client: MongoCustomClient):
    mongo_client.update_many('INVENTORY', 'collector', {},
                             {"$unset": {"plugin_info.secret_id": "",
...
def inventory_collector_migrate_schedule(mongo_client: MongoCustomClient):
    schedules = mongo_client.find('INVENTORY', 'schedule', {},
                                  {'schedule_id': 1, 'schedule': 1, 'domain_id': 1, 'collector_id': 1, 'collector': 1})
    ...
    for schedule in schedules:
        ...
        if schedule.get('schedule', {}).get('hours', []):
            ...
            if collector_id not in collector_ids:
                ...
                operations.append(UpdateOne({'_id': schedule['collector']},
...
def main(file_path):
    ...
    inventory_collector_remove_plugin_info_secret_service_account_id_provider(mongo_client)
