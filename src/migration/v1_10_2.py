...
def inventory_cloud_service_tags_refactoring(mongo_client: MongoCustomClient):
    items = mongo_client.find(
        "INVENTORY", "cloud_service", {}, {"provider": 1, "tags": 1}
    )
    operations = []
    for item in items:
        ...
        if isinstance(item["tags"], list):
            operations.append(
                UpdateOne(
                    ...
                    {
                        "$set": {
                            "tags": _change_tags_to_list_of_dict(
                                _change_tags(item["tags"]), provider
                            )
                        }
                    },
                )
            ...
            operations.append(
                UpdateOne(
                    ...
                    {
                        "$set": {
                            "tags": _change_tags_to_list_of_dict(item["tags"], provider)
                        }
                    },
                )
            )
...
@print_log
def inventory_cloud_service_delete_vm_instance_with_specific_plugin_id(
...
def identity_service_account_set_additional_fields(mongo_client: MongoCustomClient):
    mongo_client.update_many(
        "IDENTITY",
        "service_account",
        {"service_account_type": {"$ne": "TRUSTED"}},
...
def inventory_record_delete_wrong_records(mongo_client: MongoCustomClient):
    ...
    items = mongo_client.find(
        "INVENTORY",
        "record",
        {"cloud_service_id": {"$in": cloud_service_ids}},
        {"_id": 1},
    )
    for item in items:
        operations.append(DeleteOne({"_id": item["_id"]}))
...
def _change_tags_to_list_of_dict(dict_values: dict, provider: str) -> list:
    ...
    for key, value in dict_values.items():
        new_tag = {"key": key, "value": value, "type": "MANAGED", "provider": provider}
        tags.append(new_tag)
    return tags
