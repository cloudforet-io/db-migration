...
@print_log
def monitoring_alert_refactor_alert_number_by_domain_id(
    ...
    for domain_id in domain_ids:
        target_filter = {"domain_id": domain_id, "alert_number_str": {"$exists": False}}
        alerts = mongo_client.find(
            "MONITORING", "alert", target_filter, {"_id": 1, "created_at": 1}
        ).sort("created_at", 1)
        ...
        if alerts:
            ...
            _LOGGER.info(
                f"alert_number changed (domain_id: {domain_id} / number: {alert_number})"
            )
    ...
    if valid_records:
        ...
        _LOGGER.info(
            f"alert_number collection created (record count: {len(valid_records)})"
        )


@print_log
def monitoring_escalation_policy_change_scope_from_global_to_domain(
    mongo_client: MongoCustomClient,
):
...
def inventory_cloud_service_refactor_data_structure(mongo_client: MongoCustomClient):
    projection = {"provider": 1, "metadata": 1, "tags": 1, "collection_info": 1}
    target_filter = {"tags": {"$type": "array"}}

    for cloud_services in mongo_client.find_by_pagination(
        "INVENTORY", "cloud_service", target_filter, projection, show_progress=True
    ):
        operations = []
        for cloud_service in cloud_services:
            ...
            if tags and isinstance(tags, list):
                ...
                for tag in tags:
                    ...
                    new_tags[tag_provider][hashed_key] = {
                        "key": tag_key,
                        "value": tag_value,
                    }
            ...
            if len(update_fields["$set"].keys()) > 0:
                operations.append(
                    UpdateOne({"_id": cloud_service["_id"]}, update_fields)
                )
